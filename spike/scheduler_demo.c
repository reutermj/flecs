...
static ecs_entity_t ecs_id(Mass), ecs_id(Vel), ecs_id(Pos), ecs_id(AIState),
                    ecs_id(RenderData), ecs_id(Health), ecs_id(Energy);
...
int main(int argc, char **argv) {
    ...
    C(Mass);C(Vel);C(Pos);C(AIState);C(RenderData);C(Health);C(Energy);
    ...
    ecs_entity_t g = ecs_system(world, {.entity=ecs_entity(world,{.name="Gravity"}), .multi_threaded=true,
        .query.terms={{ecs_id(Mass),.inout=EcsIn},{ecs_id(Vel),.inout=EcsInOut}}, .callback=Gravity});
    ecs_entity_t m = ecs_system(world, {.entity=ecs_entity(world,{.name="Move"}), .multi_threaded=true,
        .query.terms={{ecs_id(Vel),.inout=EcsIn},{ecs_id(Pos),.inout=EcsOut}}, .callback=Move});
    ecs_entity_t a = ecs_system(world, {.entity=ecs_entity(world,{.name="AI"}), .multi_threaded=true,
        .query.terms={{ecs_id(Pos),.inout=EcsIn},{ecs_id(AIState),.inout=EcsOut}}, .callback=AI});
    ecs_entity_t r = ecs_system(world, {.entity=ecs_entity(world,{.name="Render"}), .multi_threaded=true,
        .query.terms={{ecs_id(Pos),.inout=EcsIn},{ecs_id(RenderData),.inout=EcsOut}}, .callback=Render});
    ecs_entity_t d = ecs_system(world, {.entity=ecs_entity(world,{.name="Decay"}), .multi_threaded=true,
        .query.terms={{ecs_id(Health),.inout=EcsInOut}}, .callback=Decay});
    ecs_entity_t s = ecs_system(world, {.entity=ecs_entity(world,{.name="Spin"}), .multi_threaded=true,
        .query.terms={{ecs_id(Energy),.inout=EcsInOut}}, .callback=Spin});

    ecs_scheduler_t *sched = scheduler_new(world, threads);
    scheduler_add(sched, g); scheduler_add(sched, m); scheduler_add(sched, a);
    scheduler_add(sched, r); scheduler_add(sched, d); scheduler_add(sched, s);
    ...
    float exp_vel = 2*F, exp_pos = F*(F+1), exp_ai = exp_pos, exp_rd = 2*exp_pos,
          exp_health = 1000 - F, exp_energy = F;

    int bad = 0;
    ecs_query_t *q = ecs_query(world,{.terms={
        {ecs_id(Vel)},{ecs_id(Pos)},{ecs_id(AIState)},{ecs_id(RenderData)},
        {ecs_id(Health)},{ecs_id(Energy)}}});
    ecs_iter_t it = ecs_query_iter(world, q);
    while (ecs_query_next(&it)) {
        Vel *v=ecs_field(&it,Vel,0); Pos *p=ecs_field(&it,Pos,1); AIState *ai=ecs_field(&it,AIState,2);
        RenderData *rd=ecs_field(&it,RenderData,3); Health *h=ecs_field(&it,Health,4); Energy *en=ecs_field(&it,Energy,5);
        for (int i=0;i<it.count;i++){
            if (v[i].v!=exp_vel||p[i].v!=exp_pos||ai[i].v!=exp_ai||rd[i].v!=exp_rd||
                h[i].v!=exp_health||en[i].v!=exp_energy) bad++;
        }
    }
    printf("\nframes=%d N=%d  expected: vel=%.0f pos=%.0f ai=%.0f rd=%.0f health=%.0f energy=%.0f\n",
        frames, N, exp_vel, exp_pos, exp_ai, exp_rd, exp_health, exp_energy);
